...
class Elution:
    ...
    def get_data(self):
        xArr, yArr = Elution.loadDataSet('dynamic-elution.xlsx')
        Elution.drawScatterDiagram(xArr, yArr)
        # array转mat
        xMat = np.mat(xArr).T;
        yMat = np.mat(yArr).T
        # 拆分集合为训练集合测试集
    ...
    def try_different_method(self, clf, x_train, y_train, x_test, y_test):
        ...
        # the plot
        Elution().drawPred(y_pred, y_test, score)
        Elution().drawPred(clf.predict(X), Y, score)

    # 回归树
    def DecisionTreeRegressTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        clf = DecisionTreeRegressor()
        Elution().try_different_method(clf, x_train, y_train, x_test, y_test)

    # 线性回归
    def LinearRegressionTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        linear_reg = linear_model.LinearRegression()
        Elution().try_different_method(linear_reg, x_train, y_train, x_test, y_test)

    # SVM支持向量机
    def SVMTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        svr = svm.SVR()
        Elution().try_different_method(svr, x_train, y_train, x_test, y_test)

    # K邻近knn
    def KNeighborsRegressorTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        knn = neighbors.KNeighborsRegressor()
        Elution().try_different_method(knn, x_train, y_train, x_test, y_test)

    # 随机森林
    def RandomForestRegressorTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        rf = ensemble.RandomForestRegressor(n_estimators=20)  # 使用20个决策树
        Elution().try_different_method(rf, x_train, y_train, x_test, y_test)

    # Adaboost自适应增强
    def AdaboostTest(self):
        x_train, y_train, x_test, y_test = Elution().get_data()
        ada = ensemble.AdaBoostRegressor(n_estimators=50)
        Elution().try_different_method(ada, x_train, y_train, x_test, y_test)
